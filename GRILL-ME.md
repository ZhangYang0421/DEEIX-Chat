...
用户在 DEEIX 上传 MP3 后，由后端自动提交阿里云 Fun-ASR。识别完成前附件不能发送；完成后可查看带时间和说话人的文本，并通过 RAG 用于聊天。

## 已确认决策

1. 前端上传单个 MP3，后端自动提交 Fun-ASR；API Key 不暴露给前端。
2. 第一版单个 MP3 上限 500MB；不安装 `ffprobe`，不做精确时长校验。
3. 每用户总文件配额 10GB。
4. 原始 MP3 长期保存；删除文件时联动删除对象、转写产物和处理信息。
...
14. 同一用户重复上传相同 `SHA256 + size` 的 MP3：成功结果直接复用；处理中复用进度；失败由用户手动重试。
...
25. 如果聊天上游拒绝当前请求：只标记当前聊天请求失败；保留 MP3 和转写；不自动重试；允许用户换问题或模型。用户端只显示中性提示，详细外部请求标识只写入受控日志。

## 最小处理流程

```text
POST /files
→ 校验 MP3 MIME/扩展名与 500MB 上限
→ 保存至 MinIO
→ file category = audio
