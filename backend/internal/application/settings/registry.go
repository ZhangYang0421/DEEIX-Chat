...
var settingSpecs = []settingSpec{
	...
	// 存储配置
	{Namespace: "storage", Key: "user_storage_quota_bytes", ValueType: "int", Default: "10737418240", Description: "用户总存储配额（管理页面按 MB 输入，内部以字节保存），0表示不限制",
	...
	{Namespace: "file", Key: "audio_max_bytes", ValueType: "int", Default: "524288000", Description: "MP3/M4A 音频单文件大小上限（管理页面按 MB 输入，内部以字节保存）",
		Validate: int64Min(1), Apply: applyField(func(c *config.Config) *int64 { return &c.FileAudioMaxBytes }, toInt64)},
	...
	{Namespace: "extract", Key: "paddle_ocr_base_url", ValueType: "string", Default: "https://paddleocr.aistudio-app.com/api/v2/ocr/jobs", Description: "PaddleOCR-VL 异步任务接口地址",
	...
	{Namespace: "extract", Key: "paddle_ocr_timeout_seconds", ValueType: "int", Default: "600", Description: "PaddleOCR-VL 异步任务总超时(秒)，默认 600s",
